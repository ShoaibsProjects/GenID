...
// UpdateIdentity is the resolver for the updateIdentity field.
...
// DeleteIdentity is the resolver for the deleteIdentity field.
...
// ConnectConnector is the resolver for the connectConnector field.
...
// DisconnectConnector is the resolver for the disconnectConnector field.
...
// SyncConnector is the resolver for the syncConnector field.
...
// SyncConnectorDelta is the resolver for the syncConnectorDelta field.
...
// RequestJustInTimeAccess is the resolver for the requestJustInTimeAccess field.
func (r *mutationResolver) RequestJustInTimeAccess(ctx context.Context, input JITAccessInput) (*WorkflowResult, error) {
	tenantID := ""
	requestedBy := input.IdentityID

	wfInput := workflow.JustInTimeInput{
		IdentityID:   input.IdentityID,
		ResourceID:   input.ResourceID,
		ResourceType: ifaceStr(input.ResourceType, ""),
		Action:       ifaceStr(input.Action, "read"),
		Reason:       input.Reason,
	...
	workflowID, err := r.Svc.StartJustInTimeWorkflow(ctx, wfInput)
	if err != nil {
		return nil, fmt.Errorf("graphql: jit access: %w", err)
	}

	status := "running"
	msg := "JIT access workflow started"
...
func ifaceStr(s *string, fallback string) string {
	if s == nil {
		return fallback
	}
	return *s
...
// ConnectorUsers is the resolver for the connectorUsers field.
...
// ConnectorGroups is the resolver for the connectorGroups field.
...
// ConnectorHealth is the resolver for the connectorHealth field.
